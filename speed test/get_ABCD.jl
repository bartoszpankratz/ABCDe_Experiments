...
ABCDname = "$(res_dir)/ABCD_$(n)_$(ξ)_$(β)_$(γ)_$(min_deg)_$(islocal).dat"
logname = "logs/ABCD_$(n)_$(ξ)_$(β)_$(γ)_$(min_deg)_$(islocal).log"

(isfile(ABCDname) || isfile(logname)) && exit()
...
try 
    n_edges, time = BenchmarkTools.@btimed begin
        #_ = ABCDGraphGenerator.sample_degrees($γ, $min_deg, $Int(ceil(sqrt(n))), $n, $max_iter)
        #_ = ABCDGraphGenerator.sample_communities($β, $Int(0.005*n), $Int(0.2*n), $n, $max_iter)
        p = ABCDGraphGenerator.ABCDParams($parse.(Int,readlines(degs)), $parse.(Int,readlines(coms)), 
                                            $mu, $xi, $isCL, $islocal)
        edges, _ = ABCDGraphGenerator.gen_graph(p)
        length(edges)
        end
    res = [n, ξ, μ, β, γ, min_deg, E_deg, n_edges, islocal, 1, time]
    open(io -> foreach(r -> println(io, r), res), ABCDname, "w")
catch err
    logger = SimpleLogger(open(logname, "w+"))
    with_logger(logger) do
        @warn @warn exception = (err, catch_backtrace()), "  for params: $(n) $(ξ) $(β) $(γ) $(min_deg) $(islocal)"
    end
end
