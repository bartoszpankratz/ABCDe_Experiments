...
int benchmark(bool excess, bool defect, int num_nodes, double  average_k, int  max_degree, double  tau, double  tau2, 
	...
	int  nmin, int  nmax, bool  fixed_range, double ca, bool cnodes, const string& nameDegreeSeq, 
    const string& nameCommunitySeq, const string& fnameNetwork, bool directed,
    ...
    ifstream degfile(nameDegreeSeq);
    if(!degfile){       
        cout<<"degree sequence not found, generating new one"<<endl;    // The file was not found.
        ...
        cout<<"degree sequence found! Reading it from a file."<<endl;      // The file was found.
        string str;
        double val;
        while (getline(degfile, str)) {
            cast_string_to_double(str, val);
            int deg = cast_int(val);
            degree_seq.push_back(deg);
        }
    }
    degfile.close();
    degfile.clear();
    ...
    deque<int> num_seq;		//  degree sequence of the nodes
    ifstream comfile(nameCommunitySeq);
    if(!comfile){ 
        cout<<"community sequence not here! Will be generated later"<<endl;
    }
    else{
        cout<<"reading community sequence from file..."<<endl;
        string str;
        double val;
        while (getline(comfile, str)) {
            cast_string_to_double(str, val);
            int com = cast_int(val);
            num_seq.push_back(com);
        }
    }
    comfile.close();
    comfile.clear();
...
int main(int argc, char * argv[]) {
	...
	benchmark(p.excess, p.defect, p.num_nodes, p.average_k, p.max_degree,
		...
		p.clustering_coeff, p.cnodes, p.nameDegreeSeq, p.nameCommunitySeq, p.fnameNetwork,
        p.directed, p.fnameCommunity, p.fnameStatistics);
