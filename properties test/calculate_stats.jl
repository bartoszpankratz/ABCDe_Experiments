...
function participation_stats(g, partition, algo)
    ...
    open(name, "a") do io
        ...
        ultra_peripheral_nodes = sum(part_coeffs[z .≤ 2.5] .< 0.05)/nv(g)
        peripheral_nodes = sum(0.05 .≤ part_coeffs[z .≤ 2.5] .< 0.62)/nv(g)
        connector_nodes = sum(0.62 .≤ part_coeffs[z .≤ 2.5] .< 0.8)/nv(g)
        kinless_nodes = sum(part_coeffs[z .≤ 2.5] .≥ 0.8)/nv(g)
